...
# --- DRIVER IMPORTS ---
try:
    from ArcadeDriver import Arcade, available_ports, wheel
except ImportError:
    # Fallback if driver missing
    class Arcade:
        LEDS = {}
...
def asset_path(filename: str) -> str:
    """Robust asset finder: Checks PyInstaller, Assets folder, and Root folder."""
...
# =========================================================
#  INTEGRATED DEBUGGER WINDOW (BTN TEST)
# =========================================================
class InputTestWindow(tk.Toplevel):
    def __init__(self, parent, controller):
        ...
        # Bind Mouse for Trackball
        self.bind('<Motion>', self.handle_mouse)
        
        # Init Hardware (Clean Slate)
        self.init_hardware()

    def init_hardware(self):
        """Set all LEDs to White on open."""
        if self.controller.is_connected():
            self.controller.cab.set_all((255, 255, 255))
            self.controller.cab.show()

    def create_top_bar(self):
        f = tk.Frame(self, bg="#1E1E1E", pady=10)
        f.pack(fill="x")
        tk.Label(f, text="TEST MODE", font=("Segoe UI", 14, "bold"), bg="#1E1E1E", fg="#00E5FF").pack(side="left", padx=20)
        
        # 1. Swap Inputs Checkbox
        self.swap_var = tk.BooleanVar(value=False)
        tk.Checkbutton(f, text="Swap P1/P2", variable=self.swap_var, 
                       bg="#1E1E1E", fg="white", selectcolor="#333333",
                       activebackground="#1E1E1E", activeforeground="white").pack(side="left", padx=10)

        # 2. Trackball Safety Toggle
        self.trackball_var = tk.BooleanVar(value=False)
        self.trackball_enable_time = 0.0

        def on_tb_toggle():
            if self.trackball_var.get():
                self.trackball_enable_time = time.time()
                
        tk.Checkbutton(f, text="Enable Trackball/Spinners", variable=self.trackball_var, 
                       bg="#1E1E1E", fg="white", selectcolor="#333333",
                       activebackground="#1E1E1E", activeforeground="white",
                       command=on_tb_toggle).pack(side="left", padx=20)
        
        tk.Label(f, text="(Close window to return to normal)", bg="#1E1E1E", fg="#888").pack(side="right", padx=20)

    def create_button_grid(self):
        c = tk.Frame(self, bg="#121212")
    ...
    def handle_pygame_event(self, event):
        if event.type == pygame.JOYBUTTONDOWN:
            real_id = self.get_swapped_id(event.joy)
            key = f"{real_id}_{event.button}"
            if key in INPUT_MAP:
                self.activate_button(INPUT_MAP[key])
    ...
    def get_swapped_id(self, original):
        return (1 if original == 0 else 0) if self.swap_var.get() else original
    ...
    def gui_flash(self, name, lock=False):
        if name in self.gui_buttons:
            ...
            if not lock:
                self.after(200, lambda: self.gui_buttons[name].configure(bg="#333333", fg="white"))
...
class ArcadeGUI_V1_2:
    ...
    def initialize_app(self):
        try:
            ...
            self.cab = Arcade(port=self.port) if self.port else Arcade()
            ...
            self.buttons = {}
            self.master_refs = [] # List to store refs to Master buttons for Profile Refresh
            self.led_state = {}
            ...
            if not self.is_connected():
                self.prompt_for_port(initial=True)
    ...
    def prompt_for_port(self, initial=False):
        ...
        def apply():
            if box.curselection():
                self.set_port(box.get(box.curselection()[0])); win.destroy()
        
        # CHANGED: "CONNECT" instead of "APPLY"
        ...
        if PYGAME_AVAILABLE:
            for event in pygame.event.get():
                ...
                if self.test_window and self.test_window.winfo_exists():
                    self.test_window.handle_pygame_event(event)
    ...
    def _title_anim_step(self):
        ...
        for i, (main, glow) in enumerate(getattr(self, "_title_letters", [])):
            ...
            glow.configure(fg=self._rgb_to_hex(int(r*0.3),int(g*0.3),int(b*0.3)))
        self._title_anim_phase = phase + 0.06
        self.root.after(30, self._title_anim_step)
    ...
    def build_banner(self):
        ...
        if os.path.exists(path) and PIL_AVAILABLE:
            ...
            print(f"DEBUG: Banner missing at {path}")
    ...
    def build_player_card(self, col, title, color, btns):
        ...
        # Capture Refs for Profile Refresh
    ...
    def build_utilities(self):
        ...
        def btn(t, c, cmd, w=10, f="white"): 
            ...
            return b
        btn("APPLY", COLORS["SYS"], self.apply_settings_to_hardware, w=10, f="black")
        sep()
        btn("ALL OFF", COLORS["DANGER"], self.all_off, w=9)
        btn("BTN TEST", COLORS["SURFACE_LIGHT"], self.open_button_test, w=9) 
        sep()
        btn("SWAP FIGHT", COLORS["P1"], self.swap_fight_buttons, w=10)
        btn("SWAP START", COLORS["P2"], self.swap_start_buttons, w=10)
        sep()
        
        btn("LED TEST", COLORS["SYS"], self.show_tester_menu, w=9, f="black")

        btn("CYCLE", COLORS["SURFACE_LIGHT"], self.start_cycle_mode, w=8)
        btn("DEMO", COLORS["SURFACE_LIGHT"], self.start_demo_mode, w=8)
        sep()
        
        # Store PORT button for Red/Green updates
    ...
    def update_status_loop(self):
        ...
        # TWEAK: Green text if connected, Dim if not
        if connected:
            self.status_lbl.config(fg=COLORS["SUCCESS"])
            self.port_btn.set_base_bg(COLORS["SUCCESS"]) # Green Port Button
        else:
            self.status_lbl.config(fg=COLORS["TEXT_DIM"])
            self.port_btn.set_base_bg(COLORS["DANGER"]) # Red Port Button
    ...
    def show_tester_menu(self, event=None):
        if not TESTER_AVAILABLE:
            messagebox.showerror("Error", "ArcadeTester.py not found.")
            return
    ...
    def run_external_test(self, test_func):
        ...
        self.animating = False
        self.mapping_mode = False
        self.diag_mode = True # Locks pulse engine
    ...
    def idle_watchdog_loop(self):
        ...
        self.root.after(5000, self.idle_watchdog_loop)
        ...
        try:
            ...
            for n, s in leds.items():
                if n in self.led_state:
                    self.led_state[n].update({'primary': tuple(s.get('primary', (0,0,0))), 'secondary': tuple(s.get('secondary', (0,0,0))), 'pulse': bool(s.get('pulse', False)), 'speed': float(s.get('speed', 1.0))})
    ...
    def refresh_gui_from_state(self):
        """Updates all visible buttons to match the internal led_state."""
        ...
        # 2. Update Master Buttons (based on the first button in their group)
        for ref in self.master_refs:
            # ref = {'btn': obj, 'group': ['P1_A', ...], 'mode': 'primary'}
